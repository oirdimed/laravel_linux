...
#   - Laravel version recommendation based on PHP version
#   - Laravel version selection
#   - Normal Laravel mode: Composer create-project
#   - Vemto mode: prepares an empty project directory
...
#   - Apache configuration validation before reload
...
Project types:
  ...
  2) Vemto
     ...
     Vemto generates the Laravel application afterwards.

The script is designed for Debian and Ubuntu.
...
if [[ ${#INSTALLED_PHP_VERSIONS[@]} -eq 0 ]]; then
    ...
    info "Installing PHP and common Laravel extensions..."
...
while true; do
    ...
    echo
    read -rp \
        "Select PHP version [default: ${#INSTALLED_PHP_VERSIONS[@]} - PHP $HIGHEST_PHP]: " \
        PHP_SELECTION
...
if [[ ${#MISSING_EXTENSIONS[@]} -gt 0 ]]; then
    ...
    read -rp \
        "Install missing PHP extensions now? [Y/n]: " \
        INSTALL_EXTENSIONS
...
case "$PHP_VER" in
    ...
    *)
        die "No supported Laravel recommendation for PHP $PHP_VER."
        ;;
...
echo "     Vemto generates the Laravel application afterwards."
echo
...
while true; do
    ...
    read -rp \
        "Enter project name (example: myapp): " \
        PROJ
...
# Expand ~ using the normal user's home directory.
if [[ "$BASE_DIR" == "~" ]]; then
...
if [[ -e "$PROJECT_DIR" ]]; then
    ...
    read -rp \
        "Continue and configure this directory? [y/N]: " \
        CONTINUE_EXISTING
...
if systemctl is-active --quiet mysql 2>/dev/null; then
    ...
    if systemctl list-unit-files 2>/dev/null |
       grep -q '^mysql.service'; then
        ...
        warn "MySQL/MariaDB was not detected."

        read -rp \
            "Install MariaDB now? [Y/n]: " \
            INSTALL_DB
        ...
        if [[ "$INSTALL_DB" =~ ^[Yy]$ ]]; then
            ...
            die "A MySQL-compatible database server is required."
        fi
    fi
fi
...
if [[ "$CREATE_DB_USER" =~ ^[Yy]$ ]]; then
    ...
    read -rsp \
        "Password for MySQL user '$DB_USERNAME' (blank = generate): " \
        DB_PASSWORD
    ...
    info "Creating dedicated database user..."

    "${MYSQL_CMD[@]}" <<SQL
CREATE USER IF NOT EXISTS
'${DB_USERNAME_SQL}'@'${MYSQL_HOST}'
...
if [[ "$PROJECT_TYPE" == "1" ]]; then
    ...
    if [[ -z "$COMPOSER_BIN" ]]; then
        ...
        if [[ "$INSTALL_COMPOSER" =~ ^[Yy]$ ]]; then
            ...
            die "Composer is required for Normal Laravel mode."
    ...
    info "Creating Laravel $LARAVEL_MAJOR..."
...
# Everything belongs to the normal developer user.
...
# Apache must be able to traverse the entire path.
...
if [[ "$PROJECT_TYPE" == "1" &&
    ...
    info "Application key generation deferred until Vemto generates Laravel."
...
for MOD in rewrite proxy proxy_fcgi setenvif; do
    ...
    if ! a2query -m "$MOD" >/dev/null 2>&1; then
        ...
        info "Module already enabled: $MOD"
...
<VirtualHost *:80>
    ...
    # Never expose Laravel environment or hidden files.
    <FilesMatch "^\.">
        Require all denied
    </FilesMatch>
...
if [[ "$HTTP_STATUS" =~ ^[23][0-9][0-9]$ ]]; then
    ...
    warn "HTTP test did not return a successful 2xx/3xx response."

    if [[ -f "/var/log/apache2/${PROJ}_error.log" ]]; then

        echo
        echo "Last Apache errors:"
        tail -n 10 "/var/log/apache2/${PROJ}_error.log" || true
